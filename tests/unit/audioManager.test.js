...
describe('AudioManager', () => {
  ...
  describe('init', () => {
    test('should set up event listeners, priming, and update icon', () => {
      // Create spy methods
      const setupEventListenersSpy = jest.spyOn(AudioManager.prototype, 'setupEventListeners').mockImplementation(() => {});
      const setupPrimingSpy = jest.spyOn(AudioManager.prototype, 'setupPriming').mockImplementation(() => {});
      const updateIconSpy = jest.spyOn(AudioManager.prototype, 'updateIcon').mockImplementation(() => {});
      ...
      expect(eventBus.emit).toHaveBeenCalledWith('audioManager:initialized', { enabled: true });
  ...
  describe('setupEventListeners', () => {
    ...
    test('should not set up event listeners when toggle elements do not exist', () => {
      ...
      expect(console.log).toHaveBeenCalledWith('AudioManager: Sound toggle elements not found, skipping');
    });
  });
  ...
  describe('setupPriming', () => {
    ...
    test('should not set up priming when click sound does not exist', () => {
      ...
      expect(console.log).toHaveBeenCalledWith('AudioManager: No click sound element, skipping priming');
    });
  ...
  describe('toggleSound', () => {
    ...
    test('should toggle sound off to on and play sound', () => {
      // Spy on playSound
      const playSoundSpy = jest.spyOn(AudioManager.prototype, 'playSound').mockImplementation(() => {});
  ...
  describe('playSound', () => {
    ...
    test('should not play sound when random check fails', () => {
      ...
      const mockMath = Object.create(global.Math);
      mockMath.random = () => 0.9; // Will be greater than playbackProbability
      global.Math = mockMath;
      ...
      expect(result).toBeNull();
      expect(mockClickSound.play).not.toHaveBeenCalled();
      expect(console.log).toHaveBeenCalledWith('AudioManager: Random check failed, not playing sound');
  ...
  describe('playClickSound', () => {
    test('should call playSound with click sound', () => {
      // Spy on playSound
      const playSoundSpy = jest.spyOn(AudioManager.prototype, 'playSound').mockImplementation(() => 'play-result');
